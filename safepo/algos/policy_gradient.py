...
class PG(PolicyGradient):
    def __init__(
            ...
            target_kl=0.03,
            train_pi_iterations=80,  # 80
            train_v_iterations=40,  # 40
            use_discount_cost_update_lag=False,
            use_cost_value_function=False,
            use_entropy=False,  # false
        ...
        Args:
            ...
            use_exploration_noise_anneal (bool): Use exploration noise anneal or not.
        ...
        # Create Environment
        self.env_id = env_id
        self.env = env = gym.make(env_id, config=env_cfg) if isinstance(env_id, str) else env_id
            ...
            if terminal or epoch_ended:
                if timeout or epoch_ended:
                    _, v, cv, _ = self.ac(torch.as_tensor(o, dtype=torch.float32),
                                          torch.ones_like(torch.as_tensor(action_mask, dtype=torch.float32), dtype=torch.float32))
                else:
                    v, cv = 0., 0.
    ...
    def update_policy_net(self, data) -> None:
        ...
        self.p_dist = self.ac.pi.dist(data['obs'])
