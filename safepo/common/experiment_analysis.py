...
class EnvironmentEvaluator(object):
    ...
    def eval_once(self):
        ...
        while not done:
            ...
            action, value, *_ = self.ac(obs, action_mask)
            x, r, done, info = self.env.step(action)
            ret += r
            costs += info.get('cost', 0.)
            episode_length += 1
        if self.env.actual_reward is not None:
            print(self.env.actual_reward)
        print(self.env.cur_cost)
        print(self.env.cur_storage_consumption)
