...
class CostEvaluation:
    ...
    def which_indexes_benefit(self, query, indexes):
        recommended_indexes = set()
        cost_without_index = self._get_cost(query)
        for index in indexes:
            self._prepare_cost_calculation(indexes, store_size=True)
            cost_with_index = self._get_cost(query)
            if cost_with_index < cost_without_index * 0.997:
