...
def get_beneficial_indexes(
        workload, indexes_per_query, cost_evaluation, detailed_query_information=False
):
    beneficial_indexes_workload = set()
    query_details = {}
    for query, indexes in zip(workload.queries, indexes_per_query):
        beneficial_indexes_query = cost_evaluation.which_indexes_benefit(query, indexes)
        beneficial_indexes_workload |= beneficial_indexes_query

    return beneficial_indexes_workload
